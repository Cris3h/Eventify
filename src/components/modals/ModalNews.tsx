...
const ModalNews: React.FC<ModalProps> = ({ modalState, setModalState }) => {
  ...
  return (
    <>
      {modalState && (
        ...
        {/* card */}
          <div className="bg-orange-500 shadow-md border rounded-xl p-2 w-full max-w-md h-2/4 ">
          ...
          <div className="bg-orange-200 border rounded-md justify-center align-center h-72 p-4 overflow-y-scroll scrollbar-custom">
            {testJSONAPIcall?.news.map((notice) => (
              <Link href={`/${notice.id}`} key={notice.id} className="cursor-pointer ">
